...
namespace PGORM.PostgreSQL
{
    ...
    public class DataAccess
    {
        ...
        public static List<T> ExecuteObjectQuery<T>(string sqlStatement,NpgsqlTransaction transaction, ObjectRelationMapper<T> ormFunction, params DbParameter[] parameters)
        {
            List<T> result = new List<T>();

            NpgsqlCommand command = new NpgsqlCommand(sqlStatement, DataAccess.Connection,transaction);
        ...
        public static void ExecuteNoneQuery(string sqlStatement,NpgsqlTransaction transaction, params DbParameter[] parameters)
        {
            NpgsqlCommand command = new NpgsqlCommand(sqlStatement, DataAccess.Connection,transaction);
            SetupParameters(command, parameters);
            command.ExecuteNonQuery();
        ...
        public static T ExecuteScalarQuery<T>(string sqlStatement,NpgsqlTransaction transaction, params DbParameter[] parameters)
        {
            return (T)ExecuteScalarQuery(sqlStatement,transaction, parameters);
        }

        public static object ExecuteScalarQuery(string sqlStatement,NpgsqlTransaction transaction, params DbParameter[] parameters)
        {
            NpgsqlCommand command = new NpgsqlCommand(sqlStatement, DataAccess.Connection,transaction);
            SetupParameters(command, parameters);
